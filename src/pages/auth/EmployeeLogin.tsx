    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (userData.userType !== 'employee') {
          toast.error("Access denied. Please use the employee account.");
          setIsLoading(false);
          return;
        }
        ...
        localStorage.setItem("employeeId", userData._id);
        if (userData.profile) {
          localStorage.setItem("userProfile", JSON.stringify(userData.profile));
        }
        
        console.log('[DEBUG][EmployeeLogin] Stored data:', {
          token: userData.token ? 'Present' : 'Missing',
          userType: 'employee',
          email: userData.email,
          name: userData.name,
          employeeId: userData._id,
          profile: userData.profile ? 'Present' : 'Missing'
        });
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-center md:justify-end h-full px-4 md:px-8 lg:pr-24">
        ...
        >
          <h2 className="text-2xl md:text-4xl font-bold mb-3">Employee Login</h2>
          <p className="text-lg md:text-xl text-gray-200 mb-6 md:mb-8">Sign in to access employee portal</p>
          ...
          <form onSubmit={handleSubmit} className={`space-y-4 md:space-y-6 transition-all ${error ? "animate-shake" : ""}`}>
            ...
            <motion.button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full py-3 md:py-4 bg-green-600 text-white font-semibold rounded-md transition mb-4 text-lg md:text-xl disabled:opacity-50"
            >
            ...
            <div className="mt-6 md:mt-8 text-center">
              <button
                ...
                className="text-green-400 hover:underline text-base md:text-lg"
              >
